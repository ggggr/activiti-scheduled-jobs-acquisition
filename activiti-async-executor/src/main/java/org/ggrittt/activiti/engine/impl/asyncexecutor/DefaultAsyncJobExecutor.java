...
/**
 * 
 * @author Guillaume Griselle
 */

public class DefaultAsyncJobExecutor implements AsyncExecutor {

	private final static Logger LOGGER = LoggerFactory.getLogger(DefaultAsyncJobExecutor.class);
	...
	public boolean executeAsyncJob(final Job job) {
		...
		if (isActive) {
			...
			try {
				...
				// When a RejectedExecutionException is caught, this means that
				// the queue for holding the jobs
				// that are to be executed is full and can't store more.
				// The job is now 'unlocked', meaning that the lock owner/time
				// is set to null,
				// so other executors can pick the job up (or this async
				// executor, the next time the
				// acquire query is executed.

				// This can happen while already in a command context (for
				// example in a transaction listener
				// after the async executor has been hinted that a new async job
				// is created)
				...
				// Job queue full, returning true so (if wanted) the acquiring
				// can be throttled
				return false;
			}
	...
	public void start() {
		...
		LOGGER.info("Starting up the default async job executor [{}].", getClass().getName());

		BasicThreadFactory threadFactory = new BasicThreadFactory.Builder().namingPattern("activiti-scheduler-thread-%d").build();
		scheduledExecutorService = Executors.newScheduledThreadPool(1, threadFactory);

		if (timerJobRunnable == null) {
			timerJobRunnable = new AcquireTimerJobsRunnable(scheduledExecutorService, this,processEngineConfiguration.getJobManager());
		}

		if (resetExpiredJobsRunnable == null) {
			resetExpiredJobsRunnable = new ResetExpiredJobsRunnable(scheduledExecutorService,this);
		}

		if (!isMessageQueueMode && asyncJobsDueRunnable == null) {
			asyncJobsDueRunnable = new AcquireAsyncJobsDueRunnable(scheduledExecutorService, this);
		}

		if (!isMessageQueueMode) {
			initAsyncJobExecutionThreadPool();
			asyncJobsDueRunnable.start();
		}

		timerJobRunnable.start();
		resetExpiredJobsRunnable.start(); 
	...
	public synchronized void shutdown() {
		...
		LOGGER.info("Shutting down the default async job executor [{}].", getClass().getName());
		...
		if (executorService != null) {
			...
			try {
				if (!executorService.awaitTermination(secondsToWaitOnShutdown, TimeUnit.SECONDS)) {
					LOGGER.warn("Timeout during shutdown of async job executor. "
